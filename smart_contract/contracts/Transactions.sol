...
contract realEstate {
    ...
    modifier eligibleToPayRent() {
        //only one tenant at a time can be allowed to pay rent.
    ...
    function currentTenantCheck(address _tenantcheck)
        ...
        returns (bool, uint256)
    {
        //only works if from block.number on there is just one tenant, otherwise tells untill when rent is paid.
        require(
            occupiedUntill == rentpaidUntill[tenant],
            "The entered address is not the current tenant"
        );
        ...
        eligibleToPayRent
    {
        ...
        if (rentpaidUntill[tenant] == 0 && occupiedUntill < block.number) {
            ...
            rentpaidUntill[tenant] == 0 && occupiedUntill > block.number
        ) {
            ...
            rentpaidUntill[tenant] < block.number &&
